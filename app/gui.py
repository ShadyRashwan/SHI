...
def display_why_me():
    ...
    try:
        with open("app/why_me.md", "r") as f:
        ...
        st.error("The Why Me content couldn't be found. Please try again later.")
        
        # Add a back button
        if st.button("Back to Converter"):
            # Clear query parameters using the non-experimental API
            st.query_params.clear()
            st.rerun()
...
def main():
    ...
    # Help in sidebar (collapsed by default)
    with st.sidebar:
        st.write("This app converts images to PDF.")
